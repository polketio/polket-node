...
impl sc_executor::NativeExecutionDispatch for ExecutorDispatch {
	...
	fn dispatch(method: &str, data: &[u8]) -> Option<Vec<u8>> {
		polket_runtime::api::dispatch(method, data)
	}

	fn native_version() -> sc_executor::NativeVersion {
		polket_runtime::native_version()
	}
}
...
> {
	...
	Ok(sc_service::PartialComponents {
		...
		select_chain,
		transaction_pool,
		other: (rpc_extensions_builder, import_setup, slot_duration, telemetry),
	})
}
...
pub fn new_full(mut config: Configuration) -> Result<TaskManager, ServiceError> {
	let sc_service::PartialComponents {
		...
		select_chain,
		transaction_pool,
		other: (rpc_extensions_builder, import_setup, slot_duration, mut telemetry),
	} = new_partial(&config)?;
